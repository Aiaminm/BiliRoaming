...
class BangumiSeasonHook(classLoader: ClassLoader) : BaseHook(classLoader) {
                ...
                if (instance.generalResponseClass?.isInstance(body) == true ||
                    ...
                    if (url.startsWith("https://app.bilibili.com/x/v2/search/type") || url.startsWith(
                        ...
                        if (code != 0) {
                            ...
                            code = getUrlContent?.toJSONObject()?.getIntFieldOrNull("code")
                            if (code != 0) {
    ...
    private fun fixSpace(data: Any?, mid: String, getUrlContent: String): Any? {
        ...
        val sex = content.optString("sex")
        val rank = content.optString("rank")
        val face = content.optString("face")
        val sign = content.optString("sign")
        val fans = content.optInt("fans").toString()
        val friend = content.optInt("friend").toString()
        val attention = content.optInt("attention").toString()
        val current_level = content.optJSONObject("level_info")?.optInt("current_level").toString()
        val current_min = content.optJSONObject("level_info")?.optInt("current_min").toString()
        val current_exp = content.optJSONObject("level_info")?.optInt("current_exp").toString()
        val next_exp = content.optJSONObject("level_info")?.optInt("next_exp").toString()
        val official_verify_type = content.optJSONObject("official_verify")?.optInt("type").toString()
        val official_verify_desc = content.optJSONObject("official_verify")?.optString("desc")
